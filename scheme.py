...
class Frame:
    ...
    def make_child_frame(self, formals, vals):
        ...
        if len(formals) == len(vals):
            ...
            raise SchemeError('number of arguement values does not match number of formal parameters')
        # END PROBLEM 11
        return child
...
class MacroProcedure(LambdaProcedure):
    ...
    def eval_call(self, operands, env):
        ...
        # BEGIN Problem 21
        eval_operands = complete_eval(self.apply(operands, env))
        return scheme_eval(eval_operands, env)
        # END Problem 21
...
def do_and_form(expressions, env):
    ...
    if expressions is nil:
        return True
    while expressions.second is not nil:
        eval_subexpr = scheme_eval(expressions.first, env, False)
        if scheme_falsep(eval_subexpr):
            return eval_subexpr
        expressions = expressions.second
    return scheme_eval(expressions.first, env, True)
    # END PROBLEM 13
...
def scheme_reduce(fn, lst, env):
    ...
    while lst is not nil:
        alue = complete_eval(fn.apply(scheme_list(value, lst.first), env))
        lst = lst.second
    return value
